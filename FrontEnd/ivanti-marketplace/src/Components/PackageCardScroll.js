    ...
    return(
        <>
            ...
            <div className="package" ref={package_} style={props.style}>
                <button className="button-scroll" id="left-button" ref={leftButton} onClick={scrollLeft}>{t('Left')}</button>
                <button className="button-scroll" id="right-button" ref={rightButton} onClick={scrollRight}>{t('Right')}</button>
                    ...
                    {card.map((item, index) => {
                        return (
                            <div key={index} className="packagecard-container">
                                <PackageCard title={item.title} category={item.category} id={item.id}/>
                            </div>
                        )
                    })}
                </div>
